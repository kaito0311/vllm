...
class VisionLanguageModel(nn.Module):
    ...
    def generate(self, input_ids, images, attention_mask=None, max_new_tokens=5, top_k=50, top_p=0.9, temperature=0.5, greedy=False):
        ...
        first_token_embed = token_embd.clone()
        ...
        for _ in range(max_new_tokens):
            ...
            # print("newly_generated_ids_list: ", newly_generated_ids_list)
            # next_token_embed = self.decoder.token_embedding(next_token_id) # [B, 1, D_lm]

            next_token_embed = self.decoder.token_embedding(torch.cat(newly_generated_ids_list, dim=1))
            next_token_embed = torch.concat([first_token_embed, next_token_embed], dim=1)
